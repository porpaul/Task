# Export all READY tasks with "SL886" in the name, with fallback + detailed report
$OutDir = 'C:\project\backup\tasks\SL886'
if (-not (Test-Path $OutDir)) { New-Item -Path $OutDir -ItemType Directory -Force | Out-Null }

$tasks = Get-ScheduledTask | Where-Object {
    $_.TaskName -like '*SL886*' -and $_.State -eq 'Ready'
}

if (-not $tasks) {
    Write-Output "No SL886 tasks in State=Ready."
    return
}
...
Write-Output "  Total READY SL886 tasks: $($tasks.Count)"
